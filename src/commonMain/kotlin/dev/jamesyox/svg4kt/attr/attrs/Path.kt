...
import dev.jamesyox.svg4kt.tags.AnimateMotion
import dev.jamesyox.svg4kt.tags.TextPath
import dev.jamesyox.svg4kt.util.PathBuilder
import dev.jamesyox.svg4kt.util.pathBuilder
 ...
 * [d] geometric property for the [dev.jamesyox.svg4kt.tags.Path] element. The effect of a motion path animation is a
...
/**
 * For [TextPath], `path` defines the path onto which the glyphs of a [dev.jamesyox.svg4kt.tags.Text] element will be
 * rendered. An empty string indicates that there is no path data for the element. This means that the text
 * within the [TextPath] element does not render or contribute to the bounding box of the [dev.jamesyox.svg4kt.tags.Text]
...
/**
 * Unsafe version of path. Functionality depends on what [dev.jamesyox.svg4kt.SvgTag] is the parent.
 */
context(
    ac: AttributeConsumer,
